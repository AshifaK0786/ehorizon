...
export function EventRegisterForm({ event }: { event?: any }) {
  ...
  const onSubmit = async (ev: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const payload = { event: event?.slug, teamName, person, team, screenshots: screenshots.map((f) => f.name) };
      console.log("Registration payload:", payload);
      setError("");
      alert("Registration submitted locally. Check console for payload. Implement backend submit as needed.");
      navigate(-1);
    } catch (err) {
        ...
        <div className="mt-4 rounded-3xl border border-yellow-600/25 bg-white/5 backdrop-blur-xl p-6 md:p-8">
          <form onSubmit={onSubmit} className="grid gap-6">
            ...
            <section className="space-y-4">
              <h2 className="text-xl font-extrabold text-white">Team Lead (You)</h2>
